...
# 抑制训练时常见第三方/库内 UserWarning，保持日志简洁
...
from mmdet3d.core.hook import TorchProfilerHook  # noqa: F401
...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default="projects/RoadOcc/configs/roadocc_c_4x4_24e_mf3.py", help='train config file path')
    ...
    validate_group.add_argument(
        ...
        dest='validate',
        action='store_true',
        help='evaluate checkpoints during training (default: enabled)')
    validate_group.add_argument(
        '--no-validate',
        dest='validate',
        action='store_false',
...
def _maybe_adjust_dist_backend(args, cfg):
    ...
    if want_gloo:
        cfg.dist_params['backend'] = 'gloo'
        warnings.warn(
            'Distributed backend is GLOO (config use_gloo_dist=True or '
            'OCC_V2X_DIST_BACKEND=gloo). Slower than NCCL but stable on stacks '
            'where NCCL fails; otherwise set use_gloo_dist=False, unset the env, '
            'and use dist_params backend nccl.',
            UserWarning)
...
def main():
    ...
    # work_dir is determined in this priority: CLI > config filename.
    # We intentionally ignore `work_dir` in config files to keep outputs
    # consistently under `work_dirs/`.
    if args.work_dir is not None:
        cfg.work_dir = args.work_dir
    else:
        cfg.work_dir = osp.join('./work_dirs',
                                osp.splitext(osp.basename(args.config))[0])
    if os.getenv('STCROADOCC_PROFILE_TRAIN') == '1':
        # Opt-in profiling only. This is intentionally not stored in the
        # experiment config and has no effect on an ordinary training launch.
        cfg.custom_hooks = list(cfg.get('custom_hooks', []))
        cfg.custom_hooks.append(
            dict(type='TorchProfilerHook', priority='HIGHEST',
                 output_dir=osp.join(cfg.work_dir, 'torch_profiler')))
    ...
    if args.launcher == 'none':
        ...
        dist_params.setdefault('timeout', datetime.timedelta(hours=3))
