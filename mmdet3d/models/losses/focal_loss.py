...
def sigmoid_focal_loss(pred,
    ...
    # "weighted_loss" is not applicable.  Keep this kernel in FP32: its
    # half-precision path can overflow on the static occupancy branch while
    # the surrounding network remains under AMP.
...
class CustomFocalLoss(nn.Module):
    ...
    def __init__(self,
        ...
        xy, yx = torch.meshgrid(
            [torch.arange(bev_h_) - bev_h_ / 2, torch.arange(bev_w_) - bev_w_ / 2])
    ...
    def forward(self,
        ...
        c = self.c.to(pred.device, dtype=pred.dtype)[None, :, :, None].repeat(B, 1, 1, D).reshape(-1)
