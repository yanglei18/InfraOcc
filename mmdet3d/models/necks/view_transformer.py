...
class LSSViewTransformer(BaseModule):
    ...
    def forward(self, input, depth_from_lidar=None):
        ...
        if self.with_cp:
            x = _checkpoint(self.depth_net, x)
        else:
            x = self.depth_net(x)
    ...
    def forward(self, x, mlp_input, stereo_metas=None):
        ...
        if not stereo_metas is None:
            if stereo_metas['cv_feat_list'][0] is None:
                ...
                # The key-frame stereo feature is part of the trainable image
                # path. The caller already wraps historical frames in
                # no_grad(), so keeping this operation differentiable here
                # restores the gradient from the depth/stereo branch to the
                # first ResNet/FPN outputs without retaining history frames.
        ...
        if self.with_cp:
            depth = _checkpoint(self.depth_conv, depth)
        else:
...
class DepthAggregation(nn.Module):
    ...
    def forward(self, x):
        x = _checkpoint(self.reduce_conv, x)
        short_cut = x
        x = _checkpoint(self.conv, x)
        x = short_cut + x
        x = self.out_conv(x)
        return x
