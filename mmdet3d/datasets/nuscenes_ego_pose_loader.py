...
from nuscenes.utils.splits import train as nusc_train
from nuscenes.utils.splits import val as nusc_val
from nuscenes.utils.splits import test as nusc_test
...
class nuScenesDataset(Dataset):
    def __init__(self, nusc, nusc_split):
        ...
        if self.nusc_split == "train":
            ...
            fallback_split_scenes = nusc_train
        elif self.nusc_split == "val":
            split_scenes = splits["val"]
            fallback_split_scenes = nusc_val
        else:
            split_scenes = splits["test"]
            fallback_split_scenes = nusc_test

        # Some projects override `mmdet3d/datasets/splits.py` for non-nuScenes
        # datasets. If those split names do not exist in current nuScenes meta,
        # fallback to official nuScenes splits to avoid empty valid_index.
        scene_names = {scene["name"] for scene in scenes}
        if len(set(split_scenes).intersection(scene_names)) == 0:
            split_scenes = fallback_split_scenes
        ...
        for scene in scenes:
            ...
            scene_ref_indices = list(range(valid_start_index, valid_end_index))
            self.valid_index += scene_ref_indices
            self._scene_ref_indices[scene_token] = scene_ref_indices

        assert len(self.sample_tokens) == len(self.scene_tokens) == len(self.flip_flags) == len(self.timestamps)
        self.sample_token_to_dataset_idx = {
            self.sample_tokens[ref_index]: dataset_idx
            for dataset_idx, ref_index in enumerate(self.valid_index)
        }
    ...
    def __getitem__(self, idx):
        ...
        for curr_index in self._scene_ref_indices[ref_scene_token]:
    ...
    def get_output_origin_by_sample_token(self, sample_token):
        dataset_idx = self.sample_token_to_dataset_idx.get(sample_token)
        if dataset_idx is None:
            return None
        if sample_token not in self._origin_cache:
            _, output_origin_tensor = self[dataset_idx]
            self._origin_cache[sample_token] = output_origin_tensor
        return self._origin_cache[sample_token]
