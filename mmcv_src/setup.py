...
def get_extensions():
    ...
    if EXT_TYPE == 'parrots':
        ...
        # Newer PyTorch releases require C++17 for extension builds.
        ...
        if is_rocm_pytorch or torch.cuda.is_available() or os.getenv(
            ...
            cuda_args = os.getenv('MMCV_CUDA_ARGS')
            extra_compile_args['nvcc'] = (
                [cuda_args, '-std=c++17'] if cuda_args else ['-std=c++17'])
        ...
        # Newer PyTorch releases require C++17 for CUDA extension builds.
        # However, in the windows environment, some standard libraries
        # will depend on c++17 or higher. In fact, for the windows
        # environment, the compiler will choose the appropriate compiler
        # to compile those cpp files, so there is no need to add the
