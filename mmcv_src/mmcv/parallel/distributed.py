...
class MMDistributedDataParallel(DistributedDataParallel):
    ...
    """
    # PyTorch 2.7 compatibility: this internal attribute may be absent on
    # some code paths. Keep a safe default for MMCV _run_ddp_forward logic.
    _use_replicated_tensor_module = False
    ...
    def _run_ddp_forward(self, *inputs, **kwargs) -> Any:
        ...
        replicated_module = getattr(self, '_replicated_tensor_module', None)
        module_to_run = (
            replicated_module
            if (use_replicated and replicated_module is not None)
            else self.module
        )
