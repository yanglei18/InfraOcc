...
_CLIP_GRAD_SUPPORTS_FOREACH = 'foreach' in inspect.signature(
    clip_grad.clip_grad_norm_).parameters
...
class OptimizerHook(Hook):
    ...
    def clip_grads(self, params):
        ...
        if len(params) > 0:
            clip_kwargs = self.grad_clip.copy()
            if (_CLIP_GRAD_SUPPORTS_FOREACH
                    and 'foreach' not in clip_kwargs):
                clip_kwargs['foreach'] = False
            return clip_grad.clip_grad_norm_(params, **clip_kwargs)
